...
order_products %>% group_by(order_id)  %>% count()%>% group_by(n) %>% count() %>% head(20)%>%
 ...
 xlab("Number of items in a Transaction") +ylab("No=umber of transactions")
```
...
```{r , insta_rules3}
rules3 <- apriori(transactions, parameter = list(supp = 0.01, conf = 0.1, maxlen=3), control = list(verbose = FALSE)) 
